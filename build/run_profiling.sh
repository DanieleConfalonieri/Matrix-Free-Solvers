...
for p in {1..6}; do
    for L in $(seq 3 $MAX_LEVEL_MB); do
        ...
        if [ ! -z "$DOFS" ] && [ ! -z "$THROUGHPUT" ] && [ ! -z "$ITERS" ] && [ ! -z "$TIME_ITER" ]; then
            echo "MatrixBased,$p,$L,$SETUP_SYSTEM_CPU,$SETUP_SYSTEM_WALL,$NUMBER_ELEMENTS,$DOFS,$DOFS_CELL,$QPOINTS_CELL,$LINEAR_SYSTEM,$ITERS,$TIME_ITER,$THROUGHPUT,$ASSEMBLE_CPU,$ASSEMBLE_WALL" >> $CSV_FILE
        else
            echo "   [!] Fallito o OOM per p=$p, Level=$L"
        fi
    done
